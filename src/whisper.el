...
(defcustom whisper-install-directory
...
(define-obsolete-variable-alias 'whisper-pre-process-hook
  'whisper-before-transcription-hook
  "0.3.0")

(defcustom whisper-before-transcription-hook
  '(whisper--check-buffer-read-only-p)
  "Hook run before whisper.el does anything."
  :type 'hook
  :group 'whisper)

(define-obsolete-variable-alias 'whisper-post-process-hook
...
(defun whisper--get-whispercpp-progress (_process output)
  ...
  (let ((marker "whisper_print_progress_callback: progress ="))
    (when
        (string-match
         (rx-to-string
          `(seq bol ,marker (* blank) (group (+ digit)) "%"))
         output)
...
(defun whisper--find-whispercpp-main ()
  ...
  (let*
      ((base
        (expand-file-name
         (file-name-as-directory whisper--install-path)))
       (old-bin-name
        (if (eq system-type 'windows-nt) "main.exe" "main"))
       (bin-name
        (if (eq system-type 'windows-nt) "whisper-cli.exe"
          "whisper-cli"))
           ...
           :sentinel (lambda (_process event)
                       ...
                       (cond ((or (string-equal "finished\n" event)
                             ...
                             ((string-match-p
                              ...
                              (if whisper--ffmpeg-input-file
                                  (error
                                   "FFmpeg failed to convert given file")
...
(defun whisper--transcribe-audio ()
  ...
  (setq whisper--transcribing-process
        (make-process
         ...
         :stderr (if
                     (and whisper-show-progress-in-mode-line
                          whisper--using-whispercpp)
                     (make-pipe-process
                      :name "whisper-stderr"
         ...
         :sentinel (lambda (_process event)
                     (unwind-protect
                         (when-let*
                             ((whisper--stdout-buffer
                               (get-buffer whisper--stdout-buffer-name))
                              (finished (and
                                         (buffer-live-p
                                          whisper--stdout-buffer)
                                         (string-equal
                           ...
                           (with-current-buffer whisper--stdout-buffer
                             ...
                             (when (= (buffer-size) 0)
                               (error
                                "Whisper command produced no output"))
                             (goto-char (point-min))
                             (run-hook-wrapped
                              'whisper-after-transcription-hook
                              (lambda (f)
                       ...
                       (unless whisper-show-progress-in-mode-line
...
(defun whisper--check-model-consistency ()
  ...
  (when (and (not (string-equal "en" whisper-language))
    ...
    (error
     "Use generic model (non .en version) for non-English languages"))

  (unless (or (= 2 (length whisper-language))
              (string-equal "auto" whisper-language))
    (error (concat
            "Unknown language shortcode. If unsure use 'auto'. For full list, see: "
  ...
  (let ((model-pattern (rx (seq bol
        ...
        (quantization-pattern
         (rx
          (or "q4_0" "q4_1" "q4_k" "q5_0" "q5_1" "q5_k" "q6_k" "q8_0"))))
    (unless (string-match-p model-pattern whisper-model)
      (error (concat "Speech recognition model " whisper-model
                     " not recognised. For the list, see: "
...
(defun whisper--model-file (quantized)
  ...
  (let ((base (concat
               (expand-file-name
                (file-name-as-directory whisper-install-directory))
               "whisper.cpp/"))
        (name
         (if quantized (concat whisper-model "-" whisper-quantize)
           whisper-model)))
  ...
  (catch 'early-return
    ...
    (let ((base (concat
          ...
          (compilation-buffer-name-function
           '(lambda (_) whisper--compilation-buffer-name)))

      (setq whisper--install-path base)

      (when (and (not (or (string-equal "interrupt\n" status)
                          (string-prefix-p
                           "exited abnormally with code" status)))
                 (not (or (file-exists-p (concat base old-bin-name)) ;; old location
                          (file-exists-p
                           (concat base "build/bin/" bin-name)))))

        (when (eq whisper-install-whispercpp 'manual)
          (error
           (format "Couldn't find whisper.cpp install at: %s" base)))

        (if
            (yes-or-no-p
             (format "Couldn't find whisper.cpp, install it at: %s ?"
                     base))
            (let ((make-commands
                   (concat
                    ...
                    "cd whisper.cpp && "
                    "CLICOLOR=0 make")))
              (setq whisper--compilation-buffer
                    (get-buffer-create
                     whisper--compilation-buffer-name))
      ...
      (when (and (not (file-exists-p (whisper--model-file nil)))
        ...
        (if
            (yes-or-no-p
             (format
              "Speech recognition model \"%s\" isn't available, download now?"
              whisper-model))
            (let ((make-commands
                   (concat
                    "cd " base " && "
        ...
        (if (not (file-exists-p (concat base "build/bin/quantize")))
            (let ((make-commands
                   (concat
                    ...
                    "./build/bin/quantize " (whisper--model-file nil)
                    " " (whisper--model-file t) " " whisper-quantize)))
              (setq whisper--compilation-buffer
                    (get-buffer-create
                     whisper--compilation-buffer-name))
              (add-hook 'compilation-finish-functions
      ...
      (when (string-equal "interrupt\n" status)
        ...
        (when
            (and (file-directory-p base)
                 (string-suffix-p "/whisper.cpp/" base))
          (if (file-exists-p (concat base "build/bin/" bin-name))
              ...
              (progn
                ...
                (delete-file
                 (concat base "models/" "ggml-" whisper-model ".bin")))
      ...
      (when (string-prefix-p "exited abnormally with code" status)
        (if (eq whisper-install-whispercpp 'manual)
            (message
             "Compilation exited abnormally, but not deleting directory because installation is manual.")
          (progn
            (delete-directory whisper--install-path t)
            (message
             "Couldn't compile whisper.cpp. Check that you have Git, a C++ compiler and CMake installed.")))
      ...
      (when (string-equal "finished\n" status)
        (unless (or whisper--ffmpeg-input-file
                    (yes-or-no-p
                     "Speech recognition model download completed, want to record audio now?"))
          (throw 'early-return nil)))
...
- When transcribing is in progress, cancels it."
  ...
  (if (process-live-p whisper--transcribing-process)
      (when
          (yes-or-no-p
           "A transcribing is already in progress, kill it?")
    ...
    (cond
     ...
     ((and (buffer-live-p whisper--compilation-buffer)
           (process-live-p
            (get-buffer-process whisper--compilation-buffer)))
      (when-let*
          ((proc (get-buffer-process whisper--compilation-buffer)))
       ...
       (pcase arg
         ...
         ('(4)
          (when-let*
              ((file
                (expand-file-name
                 (read-file-name "Media file: " nil nil t))))
      ...
      (if whisper-install-whispercpp
        ...
        (let ((command (car (whisper-command whisper--temp-file))))
          (if (or (file-exists-p command)
              ...
              (whisper--record-audio)
            (error
             (format "Couldn't find %s in PATH, nor is it a file"
                     command)))))))))
