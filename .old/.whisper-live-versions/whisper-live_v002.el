;;; -*- lexical-binding: t -*-
;;; Author: 2024-12-07 08:59:05
;;; Time-stamp: <2024-12-07 08:59:05 (ywatanabe)>
   ...
   :sentinel (lambda (_process event)
               (when (string-equal "finished\n" event)
                 (with-current-buffer (get-buffer "*whisper-temp*")
                   ...
                   (when (re-search-forward "\n\n \\(.*\\)\n\n" nil t)
                     (let ((text (match-string 1)))
                       (when (and text (buffer-live-p whisper-live--target-buffer))
                         (with-current-buffer whisper-live--target-buffer
                           (insert text " "))))))
                 (kill-buffer "*whisper-temp*")
                 (delete-file chunk-file)))))
...
(defun whisper-live-stop ()
  ...
  (message "Whisper recording stopped..."))
