;;; -*- lexical-binding: t -*-
;;; Author: 2024-12-07 11:49:31
;;; Time-stamp: <2024-12-07 11:49:31 (ywatanabe)>
...
(defcustom whisper-live-ffmpeg-input-format
  (pcase system-type
    ...
    ('windows-nt "dshow")
    (_ nil))
  "Input format for ffmpeg recording."
  :type 'string
  :group 'whisper-live)

(defcustom whisper-live-ffmpeg-input-device
  (pcase whisper-live-ffmpeg-input-format
    ("pulse" "default")
    (_ nil))
  "Input device for ffmpeg recording."
  :type 'string
  :group 'whisper-live)

;; (defcustom whisper-live-whisper-path "whisper"
;;   "Path to whisper executable."
;;   :type 'string
;;   :group 'whisper-live)


(defcustom whisper-live-whisper-path
  (or (executable-find "whisper")
      (executable-find "whisper-cpp")
      (executable-find "main")  ; whisper.cpp binary name
      "whisper")  ; fallback
  "Path to whisper executable."
  :type 'string
  :group 'whisper-live)
...
(defun whisper-live--make-command (input-file)
  "Create whisper command for INPUT-FILE."
  `(,whisper-live-whisper-path ,input-file))

(defun whisper-live--transcribe-chunk (chunk-file)
  "Transcribe a single CHUNK-FILE."
  (make-process
   :name "whisper-live-transcribing"
   :command (whisper-live--make-command chunk-file)
  ...
  (let ((chunk-file (whisper-live--generate-chunk-filename)))
    (setq whisper-live--current-process
          (make-process
           :name "whisper-live-recording"
           :command `("ffmpeg"
                     "-f" ,whisper-live-ffmpeg-input-format
                     "-i" ,whisper-live-ffmpeg-input-device
                     "-t" ,(number-to-string whisper-live-chunk-duration)
                     "-ar" "16000"
                     "-y" ,chunk-file)
           :sentinel (lambda (_process event)
...
(defun whisper-live-run ()
  ...
  (if whisper-live--current-process
      (whisper-live-stop)
    (if (executable-find whisper-live-whisper-path)
        (whisper-live-start)
...
;;           (whisper-live-start))
;;       (let ((command (car (whisper-live--make-command whisper--temp-file))))
